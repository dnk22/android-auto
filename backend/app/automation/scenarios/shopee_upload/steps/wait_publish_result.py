...
UPLOAD_LOADING_SELECTORS = [
    {"text_contains": "Đang tải"},
    {"text_contains": "đang tải"},
    {"text_contains": "Đang upload"},
    {"text_contains": "đang upload"},
    {"text_contains": "Uploading"},
    {"text_contains": "uploading"},
]


async def _is_uploading(connection) -> bool:
    """
    Check xem màn hình còn trạng thái đang tải/đang upload không.
    Mặc định ưu tiên check theo text_contains.
    """
    for selector in UPLOAD_LOADING_SELECTORS:
        try:
            found = await exists(
                connection,
                selector,
...
async def _wait_until_upload_finished(
    ...
    Nếu 2 lần liên tiếp không thấy phần tử đang tải/đang upload thì coi như upload đã xong.
    """
    max_wait_sec = float(
        payload.extra.get(
            "publish_wait_sec",
    ...
    while elapsed_sec < max_wait_sec:
        is_uploading = await _is_uploading(payload.connection)
